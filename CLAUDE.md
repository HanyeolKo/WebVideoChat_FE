...
| 2026-06-06 | 통합 관제 하네스 연동: devops.md 드리프트 수정(옛 SSH 배포·$IMAGE_TAG 버그·SSH_* 시크릿 기술 → GHCR+self-hosted+상대경로 모델로 정정), fe-harness Phase 3·contract-verification에 관제탑(contract-steward/release-coordinator) 에스컬레이션 포인터 추가, contract-spec을 관제탑 정본의 거울로 명시, README에 하네스 절 추가 | devops.md·fe-harness·contract-verification·contract-spec·README | 상위 관제탑 도입에 따른 정합성 반영 및 스킬-에이전트 드리프트 해소 |
| 2026-06-07 | 하네스 확장 2건: (1) `pr-merge-flow` 스킬 + `pr-reviewer`(opus) 에이전트 추가 — PR 생성→적대적 검수→blocker 수정(최대 2R)→조건부 자동병합(blocker 0+CI green, main 병합=운영배포라 게이트). (2) cicd-management에 `deploy-symptom-triage.md` 추가 — 배포 증상 디버깅 시 레포 밖 점검(nginx WS 프록시·TURN/STUN·CORS·env) 체크리스트 자동 생성. fe-harness 라우팅·description 갱신 | pr-reviewer.md·pr-merge-flow·cicd-management/references·fe-harness | 영상통화 디버깅에서 레포 밖 원인 누락 방지 + PR 검수·병합 자동화(안전 게이트 포함) 요청 반영 |
